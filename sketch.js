...
function draw() {
  ...
  if (World.frameCount % 150 == 0) {
   ...
   if(CGRosa.isTouching(ciclistaPrincipal)){
    ...
    text ("aperte o botão direcional para cima para reiniciar o jogo", 400, 200);
 ...
 ciclistaPrincipal.addAnimation("SahilRunning",img2CiclistaPrincipal);
    ...
    //condição de gravação para chamada de reset()
     if(estadoJogo === ENCERRAMENTO && keyDown("space")){
        restart();
}
}
}
...
function restart(){
  estadoJogo = JOGAR;
  fimdeJogo.visible = false;
  ciclistaPrincipal.addAnimation("Sahil Running",       img1CiclistaPrincipal);
  CGRosa.destroyEach();
